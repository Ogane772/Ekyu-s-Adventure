	...
	in float2 Tex : TEXCOORD)
{
    ...
    output.Normal = normalize(mul((float3) Norm, (float3x3) g_mW));
...
float4 PS(VS_OUTPUT input) : SV_Target
{
		...
		saturate(LightTexCoord2.y) == LightTexCoord2.y)
	{
		...
		//垂直の時は影を付けない
        if (abs(dot(input.Normal, (float3)g_LightFront)) > 0.85)
		{
    ...
    color.w = Dithering(g_texDecal, g_samLinear, input.Tex, g_Alpha, input.Pos);
