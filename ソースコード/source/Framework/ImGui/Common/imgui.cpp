...
/*
 ...
 A: Short explanation:
    ...
    Here's a simplified OpenGL example using stb_image.h:
        ...
        unsigned char* my_image_data = stbi_load("my_image.png", &my_image_width, &my_image_height, nullptr, 4);
...
{
    if (c > 0 && c < 0x10000)
        InputQueueCharacters.emplace_back((ImWchar)c);
}
...
bool ImGui::FocusableItemRegister(ImGuiWindow* window, ImGuiID id)
{
    ...
    if (g.ActiveId == id && g.FocusTabPressed && !(g.ActiveIdBlockNavInputFlags & (1 << ImGuiNavInput_KeyTab_)) && g.FocusRequestNextWindow == nullptr)
    {
...
{
    ImGuiWindow* window = GImGui->CurrentWindow;
    window->IDStack.emplace_back(window->GetIDNoKeepAlive(str_id));
}

void ImGui::PushID(const char* str_id_begin, const char* str_id_end)
{
    ImGuiWindow* window = GImGui->CurrentWindow;
    window->IDStack.emplace_back(window->GetIDNoKeepAlive(str_id_begin, str_id_end));
}

void ImGui::PushID(const void* ptr_id)
{
    ImGuiWindow* window = GImGui->CurrentWindow;
    window->IDStack.emplace_back(window->GetIDNoKeepAlive(ptr_id));
}

void ImGui::PushID(int int_id)
{
    ImGuiWindow* window = GImGui->CurrentWindow;
    window->IDStack.emplace_back(window->GetIDNoKeepAlive(int_id));
}
