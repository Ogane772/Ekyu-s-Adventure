            ...
            for (int i = 0; i < sequenceCount; i++)
            {
                ...
                if (localCustomHeight > 0)
                {
                   ...
                   ImRect clippingRect(rp + ImVec2(float(legendWidth), float(ItemHeight)), rp + ImVec2(canvas_size.x, float(localCustomHeight + ItemHeight)));
                    ...
                    customDraws.emplace_back({ i, customRect, legendRect, clippingRect, legendClippingRect });
                }
                else
                {
                   ...
                   compactCustomDraws.emplace_back({ i, customRect, ImRect(), clippingRect, ImRect() });
                }
                customHeight += localCustomHeight;
            }
