...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* v, float v_speed, const void* v_min, const void* v_max, const char* format, float power)
{
    ...
    if (power != 1.0f)
        IM_ASSERT(v_min != nullptr && v_max != nullptr); // When using a power curve the drag needs to have known bounds
...
bool ImGui::TempInputTextScalar(const ImRect& bb, ImGuiID id, const char* label, ImGuiDataType data_type, void* data_ptr, const char* format)
{
    ...
    if (value_changed)
    {
        value_changed = DataTypeApplyOpFromText(data_buf, g.InputTextState.InitialTextA.Data, data_type, data_ptr, nullptr);
        if (value_changed)
            MarkItemEdited(id);
    }
    return value_changed;
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const bool user_scroll_finish = is_multiline && state != nullptr && g.ActiveId == 0 && g.ActiveIdPreviousFrame == GetScrollbarID(draw_window, ImGuiAxis_Y);
    const bool user_scroll_active = is_multiline && state != nullptr && g.ActiveId == GetScrollbarID(draw_window, ImGuiAxis_Y);
    ...
    if (render_cursor || render_selection)
    {
...

        {
            ...
            int line_count = 0;
            //for (const ImWchar* s = text_begin; (s = (const ImWchar*)wcschr((const wchar_t*)s, (wchar_t)'\n')) != nullptr; s++)  // FIXME-OPT: Could use this when wchar_t are 16-bits
...
void    ImGui::EndTabBar()
{
    ...
    if (tab_bar == nullptr)
    {
        IM_ASSERT(tab_bar != nullptr && "Mismatched BeginTabBar()/EndTabBar()!");
...
ImGuiColumns* ImGui::FindOrCreateColumns(ImGuiWindow* window, ImGuiID id)
{
    ...
    window->ColumnsStorage.emplace_back(ImGuiColumns());
